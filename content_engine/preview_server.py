...
function toPreviewPath(path){
  ...
  const marker='/content_engine/';
  const idx=normalized.indexOf(marker);
  if(idx>=0){
    return '/output/' + normalized.slice(idx + marker.length);
  }
  const outputMarker='/output/';
  const outputIdx=normalized.indexOf(outputMarker);
  if(outputIdx>=0){
    return normalized.slice(outputIdx);
  }
  return '';
}
...
function render(){
  ...
  document.getElementById('grid').innerHTML=filtered.map(c=>{
    const heroImg=c.card_paths&&c.card_paths[0]?toPreviewPath(c.card_paths[0]):'';
...
function showDetail(c){
  const imgs=c.card_paths?c.card_paths.map(p=>
    `<img class="modal-img" src="${toPreviewPath(p)}" onerror="this.style.display='none'">`
  ).join(''):'';
...
init();
    ...
    def _serve_health(self):
        ...
        self.send_header("Content-Type", "application/json; charset=utf-8")
        self.end_headers()
        self.wfile.write(json.dumps({"status": "ok", "service": "content_preview"}).encode("utf-8"))
    ...
    def _regenerate(self, params):
        self.send_response(200)
        self.send_header("Content-Type", "application/json; charset=utf-8")
        self.end_headers()
        self.wfile.write(json.dumps({"ok": False, "message": "当前预览服务仅负责浏览，重新生成请运行 batch_generator.py"}).encode("utf-8"))
