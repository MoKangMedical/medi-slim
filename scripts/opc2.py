...
def at(hd, rows):
    tb = doc.add_table(rows=len(rows)+1, cols=len(hd), style="Light Grid Accent 1")
    tb.alignment = WD_TABLE_ALIGNMENT.CENTER
    for i,h in enumerate(hd): tb.rows[0].cells[i].text = h
    for ri,row in enumerate(rows,1):
        for ci,v in enumerate(row): tb.rows[ri].cells[ci].text = str(v)

# Cover
for _ in range(6): doc.add_paragraph()
p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run("MediSlim OPC 运营手册"); r.font.size = Pt(48); r.font.bold = True; r.font.color.rgb = RGBColor(7,193,96)
p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run("一人公司+AI驱动的消费医疗平台 v1.0"); r.font.size = Pt(20); r.font.color.rgb = RGBColor(100,100,100)
p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run("2026-04-04 | 贾维斯(Jarvis)AI系统"); r.font.size = Pt(12); r.font.color.rgb = RGBColor(150,150,150)
doc.add_page_break()

# TOC
gh("目录")
for t in ["一、项目概述与OPC定位","二、AI四层架构","三、人员配置与阶段规划","四、AI自动化清单",
"五、三圈产品矩阵详解","六、业务流程详解","七、全渠道运营SOP","八、资金流与财务流程",
"九、数据看板与决策体系","十、合规检查清单","十一、30天执行计划","十二、应急手册"]:
    doc.add_paragraph(t)
doc.add_page_break()

# Ch1
gh("一、项目概述与OPC定位")
doc.add_paragraph("MediSlim是AI驱动的消费医疗+健康消费品平台，借鉴美国Medvi公司(年营收4亿美金)的极简架构，结合中国特色打造从专业医疗到日常养生的全覆盖平台。")
doc.add_paragraph("OPC(One Person Company)模式核心：AI驱动90%日常运营，创始人只做关键决策。DeepSeek API 推理，三圈产品矩阵覆盖全消费场景，全外包履约消除重资产。")
doc.add_paragraph("我们不是医疗机构，是流量+技术服务公司。处方权归合作医师，药品销售归合作药房，我们做品牌、流量、AI评估、患者管理、复购运营。")
at(["维度","Medvi(美国)","MediSlim(中国)"],[["品类","单一(GLP-1)","全品类(处方+药食+保健品)"],["文化","无文化认同","中医体质辨识+药食同源"],["合规","FDA灰色地带","完全合规(持牌合作)"],["护城河","零","AI体质辨识+用户数据"],["利润率","16.2%","预估35%"],["AI能力","付费API","DeepSeek API"]])
doc.add_page_break()
...
# Ch3
gh("三、人员配置与阶段规划")
at(["阶段","时间","人数","分工"],[["阶段1","M1-M3","1人","小林=CEO+产品+合规；AI全包其余"],["阶段2","M4-M6","2人","+1运营助理(客服/社群/发货)"],["阶段3","M7-M12","3-5人","+1内容(视频/直播)+1技术(系统维护)"]])
doc.add_paragraph()
doc.add_paragraph("OPC精简逻辑：AI替代整个部门。内容→AI Agent；客服→AI Agent；数据分析→AI Agent。人只做AI做不了的事：决策、谈判、出镜、投诉处理。")
doc.add_page_break()
...
doc.add_paragraph("⚠️ 关键限制：社交媒体严禁药品营销。公域只推药食同源+保健品。处方药仅通过互联网医院渠道，由医师推荐。私域(企微1对1)引导到互联网医院评估。")
...
doc.add_paragraph("核心差异化竞争力。食品级产品，合规门槛极低(SC许可证)，文化认同度极高，复购率80%+，毛利率60-70%。")
...
for i,(s,d) in enumerate([("触达","社交媒体广告/社群推荐/口碑传播"),("落地","H5落地页/企微社群/朋友推荐"),("评估","AI健康问卷5分钟：处方类->症状+禁忌 | 药食同源->体质辨识 | 保健品->营养需求"),("方案","个性化健康方案(AI生成+医师审核)"),("下单","微信支付/支付宝付款"),("履约","处方->互联网医院开方->药房->顺丰 | 药食同源/保健品->工厂/仓->顺丰"),("随访","企微1对1跟踪+每周打卡+AI调方案"),("复购","Day 21提醒->一键续费->自动发货->循环"),("裂变","邀请好友->双方优惠->社群运营->滚雪球")],1):
    doc.add_paragraph("Step %d %s：%s" % (i,s,d))
...
at(["时间","动作","content"],[["Day 0","加好友","欢迎语+AI评估链接"],["Day 1","推报告","AI评估报告+推荐产品"],["Day 3","跟反馈","使用感受+解答疑问"],["Day 7","邀入群","产品群+首购优惠"],["Day 14","效果跟进","打卡反馈+效果记录"],["Day 21","复购提醒","一键续费链接"],["Day 28","续费优惠","续费折扣+升级建议"]])
...
for i in ["今日新增线索数","今日新订单数","今日收入(元)","线索->订单转化率","本月MRR(月经常性收入)"]:
    doc.add_paragraph(i, style="List Bullet")
...
doc.add_paragraph()
at(["version","date","author","content"],[["v1.0","2026-04-04","Jarvis AI","12章完整OPC运营手册"]])

path = "/root/medi-slim/docs/MediSlim-OPC运营手册-v1.0.docx"
doc.save(path)
print("DONE %dKB" % (os.path.getsize(path)//1024))
